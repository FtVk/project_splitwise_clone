            ...
            for name, data in group_data["groups"].items():
                ...
                for edge in data["edges"]:
                    groups[name].graph.add_transaction(
                        edge["from"], edge["to"], edge["amount"], edge["category"], edge["timestamp"], edge["explanation"], edge["transaction_id"]
                    )
            ...
            name: {
                ...
                "edges": [
                    {"from": from_user, "to": transaction["to"], "amount": transaction["amount"],
                     "category": transaction["category"], "timestamp": transaction["timestamp"], "explanation": transaction["explanation"], "transaction_id": transaction["transaction_id"]}
...
@app.route("/groups/<group_name>/transactions/<transaction_id>", methods=["DELETE"])
def delete_transaction(group_name, transaction_id):
    """Delete a specific transaction from a group using its transaction ID."""
    group = groups.get(group_name)
    if not group:
        return jsonify({"error": f"Group '{group_name}' not found."}), 404
    
    # Use the transaction_id from the URL directly
    success = group.graph.delete_transaction(transaction_id)
